...
// 2026-09-04: 이 창을 닫아도 뒤의 서버(7860/8080/8081)가 안 꺼지던 문제를, 밑색(flatcolor)
// 버튼과 똑같은 방식으로 고쳤다 - 전용 프로필 없이 크롬을 띄우면 이미 떠 있는 기본 프로필
// 크롬에게 창 하나만 더 열라고 위임해버려서 이 child 프로세스가 곧장 끝나버리고, 그러면
// 아래 'exit' 리스너도 "창을 닫을 때"가 아니라 "위임하고 곧장 끝날 때" 발동해서 서버 자동
// 종료가 동작하지 않는다. 전용 프로필(WEBTOON3D_PROFILE_DIR)을 쓰면 완전히 독립된 새
// 프로세스가 뜨고, 그 프로세스의 수명이 곧 이 창의 수명과 일치하므로 종료 시점을 정확히
// 알 수 있다.
const WEBTOON3D_PROFILE_DIR = path.join(WEBTOON3D_DIR, '.appwindow_reflib');
// 밑색(flatcolor, AI 보조 선화 채색) 앱 - ComfyUI/웹툰3D와 같은 "서버 상태 확인 -> 필요하면
// 켜기 -> 창 열기" 패턴. server.py가 0.0.0.0으로 열려 있어 폰(Tailscale)에서도 그대로
// 접속되므로, 이 버튼이 서버를 확실히 띄워주기만 하면 모바일 접속용으로도 충분하다.
// 서버 시작용 배치는 기존 실행.bat(서버+자체 크롬 창까지 같이 여는 파일)을 그대로 쓰지 않고
// 서버만실행.bat(서버만 백그라운드로 켜고 바로 끝나는 파일)을 새로 만들어 썼다 - 안 그러면
// 실행.bat이 연 크롬 창과, 아래에서 우리가 위치까지 맞춰서 여는 크롬 창이 중복으로 뜬다.
// 2026-09-02 추가.
const FLATCOLOR_DIR = 'D:\\AI-Workflow\\flatcolor';
const FLATCOLOR_BAT = path.join(FLATCOLOR_DIR, '서버만실행.bat');
const FLATCOLOR_URL = 'http://127.0.0.1:8790';
// 이 버튼 전용 크롬 프로필(폴더가 없으면 크롬이 처음 실행 시 알아서 만든다). 실행.bat도
// 자기 전용 프로필(flatcolor\.appwindow)을 따로 쓰는데, 그것과 일부러 다른 폴더를 쓴다 -
// 두 진입 경로가 동시에 열릴 수 있어서 같은 프로필을 공유하면 나중에 연 쪽이 먼저 뜬
// 프로필의 크롬에게 창 하나만 더 열라고 위임해버려(프로필당 인스턴스는 하나) 지금 고치는
// 문제(창 크기가 다르게 나옴 / 닫아도 서버가 안 꺼짐)가 그대로 재현된다. 전용 폴더를 따로
...
app.whenReady().then(() => {
  ...
  // toggleFocusedWindowTopmost 참고. (이전 조합키들도 이 PC의 다른 프로그램이 차례로 선점해서 등록이
  // 조용히 실패했던 이력이 있다 - Electron은 이미 선점된 전역 단축키를 등록하려 해도 에러를 던지지
  // 않는다. 2026-08-24에 지금의 Ctrl+Alt+Shift+F9로 바꾸면서, 콘솔을 안 봐도 바로 알 수 있도록 등록
  // 성공/실패를 앱 시작 시 알림으로도 띄우게 했다. 앞으로 또 안 먹으면 똑같은 선점 문제일 가능성이
  // 높으니 다른 조합으로 바꿔볼 것.)
...
// 어디에 뜨든 상관없이 정확히 찾는다.
// (2026-09-06, 사용자 실측 -- "레퍼런스앱 앱링크들이 앱들을 수정한 뒤에 열면
...
// 밑색(flatcolor) 서버가 살아있는지 확인 - ComfyUI와 같은 방식(가벼운 GET, 에러/타임아웃이면 죽은 것으로 판단).
function checkFlatcolorAlive() {
  return new Promise((resolve) => {
    const req = http.get(FLATCOLOR_URL, { timeout: 1500 }, (res) => {
      res.resume();
      resolve(true);
    });
    req.on('error', () => resolve(false));
...
async function waitForFlatcolor(maxWaitMs) {
  const start = Date.now();
  while (Date.now() - start < maxWaitMs) {
    if (await checkFlatcolorAlive()) return true;
    await new Promise((r) => setTimeout(r, 1000));
  }
  return false;
}

ipcMain.handle('open-flatcolor', async () => {
  try {
    const alreadyRunning = await checkFlatcolorAlive();
    if (!alreadyRunning) {
      if (!fs.existsSync(FLATCOLOR_BAT)) {
        return { success: false, error: '밑색 서버 실행 파일을 찾을 수 없습니다: ' + FLATCOLOR_BAT };
      }
      const serverProc = spawn('cmd.exe', ['/c', FLATCOLOR_BAT], {
        cwd: FLATCOLOR_DIR, detached: true, stdio: 'ignore'
      });
      serverProc.unref();
      const ready = await waitForFlatcolor(60000);
      if (!ready) {
        return { success: false, error: '밑색 서버가 60초 안에 켜지지 않았습니다. 켜지는 중이라면 잠시 후 다시 눌러주세요.' };
      }
    }
    const exePath = await resolveExePath('chromeExe', DEFAULT_CHROME_EXE, '크롬 실행 파일(chrome.exe)을 선택하세요');
    if (!exePath) return { success: false, error: '크롬 실행 파일 위치를 찾지 못함' };
    ...
    const child = spawn(exePath, [
      ...
      `--window-size=${w},${h}`,
      `--window-position=${x},${y}`,
      `--user-data-dir=${FLATCOLOR_PROFILE_DIR}`,
      '--disable-background-mode'
    ], { detached: true, stdio: 'ignore' });
    // 2026-09-03 수정: --user-data-dir 없이 크롬을 띄우면 이미 떠 있는 기본 프로필 크롬에게
    // 창 하나만 더 열라고 위임하고 이 child 프로세스는 곧바로 끝나버려서 (1) window-size/
    // window-position/force-dark-mode 같은 옵션이 무시되고(그래서 실행.bat으로 연 창과 크기가
    // 달라 보였음) (2) 아래 'exit' 리스너도 "창을 닫을 때"가 아니라 "위임하고 곧장 끝날 때"
    // 발동해버려 서버 자동 종료가 동작하지 않았다. 전용 프로필(FLATCOLOR_PROFILE_DIR)을 쓰면
    // 완전히 독립된 새 프로세스가 뜨고, 그 프로세스가 곧 이 창의 수명과 일치하므로 종료
    // 시점을 정확히 알 수 있다. --force-dark-mode는 실행.bat 쪽 창과 모양을 맞추기 위해
    // 제거했다.
    child.on('exit', () => {
      // 실행.bat과 같은 방식: 이 창이 닫히면 포트 8790을 물고 있는 프로세스를 찾아 종료한다.
      // 채색 작업이 진행 중이었다면 그 작업은 강제 종료되어 결과가 남지 않을 수 있다.
      try {
        const out = spawnSync('cmd.exe', ['/c', 'netstat -ano | findstr ":8790 "'], { encoding: 'utf8' });
        const pids = new Set();
        (out.stdout || '').split(/\r?\n/).forEach((line) => {
...
ipcMain.handle('open-webtoon3d', async () => {
  try {
    ...
    const child = spawn(exePath, [
      ...
      `--window-size=${w},${h}`,
      `--window-position=${x},${y}`,
      `--user-data-dir=${WEBTOON3D_PROFILE_DIR}`,
      '--disable-background-mode'
    ], { detached: true, stdio: 'ignore' });
    // 2026-09-04: force-dark-mode/WebContentsForceDark 플래그를 뺐다 - 위쪽(2197줄 근처)
    // 주석에 이미 적혀 있듯 이 플래그들은 대체로 효과가 없고(거의 항상 무시됨), 밑색
    // 버튼이 실제로 검증된 조합(--user-data-dir + --disable-background-mode)과 최대한
    // 똑같이 맞춰서 창을 닫을 때 서버가 안 꺼지는 문제의 변수를 줄였다.
    child.on('exit', () => {
      // 이 창이 닫히면 웹툰 3D 관련 포트(7860 웹앱, 8080 Hunyuan-2mv, 8081 Hunyuan-2.1)를
      // 물고 있는 프로세스를 모두 찾아 종료한다. 작업이 진행 중이었다면 강제 종료되어
      // 결과가 남지 않을 수 있다. (flatcolor 버튼과 동일한 방식)
      try {
        const ports = ['7860', '8080', '8081'];
        const pids = new Set();
        for (const port of ports) {
          const out = spawnSync('cmd.exe', ['/c', `netstat -ano | findstr ":${port} "`], { encoding: 'utf8' });
          (out.stdout || '').split(/\r?\n/).forEach((line) => {
            const parts = line.trim().split(/\s+/);
            const pid = parts[parts.length - 1];
            if (/^\d+$/.test(pid) && pid !== '0') pids.add(pid);
        ...
        // 서버 종료 실패는 무시 - 다음에 웹툰 3D 버튼을 누르면 살아있는 서버를 그대로
        // 재사용하거나 새로 켠다.
      }
    });
    positionNewMiniWindow(beforeHandles, x, y, w, h);
